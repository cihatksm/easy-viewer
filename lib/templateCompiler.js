...
function validateExpression(ast) {
    const allowed = new Set([
        ...
        'CallExpression', 'TemplateLiteral', 'ArrowFunctionExpression', 'TemplateElement'
    ]);
    let unsafe = false;
    (function walk(node) {
        ...
        switch (node.type) {
            ...
            case 'MemberExpression':
                ...
                break;
            case 'CallExpression':
                // allow simple calls like arr.map(...), func(...)
                walk(node.callee);
            ...
            case 'ArrowFunctionExpression':
                // only allow concise arrow functions with expression bodies
                if (!node.body) { unsafe = true; return; }
                if (node.body.type === 'BlockStatement') { unsafe = true; return; }
                // ensure params are simple identifiers
                for (const p of node.params) {
                    if (!p || p.type !== 'Identifier') { unsafe = true; return; }
                    if (/^(process|require|global|Function|eval|window|module|this|constructor|__proto__|prototype)$/.test(p.name)) unsafe = true;
                }
                walk(node.body);
                break;
...
function compile(template, options = {}) {
    const doubleRaw = !!options.doubleBraceRaw;
    const cacheKey = template + '\0' + (doubleRaw ? '1' : '0');
    if (templateCache.has(cacheKey)) return templateCache.get(cacheKey);

    const parts = [];
    // Support both {{ ... }} (escaped) and {{{ ... }}} (raw, but sanitized)
    ...
    while ((match = regex.exec(template)) !== null) {
        ...
        let code;
        let isRaw = false;
        if (match[2] !== undefined) { code = match[2].trim(); isRaw = true; }
        else {
            code = match[3].trim();
            // Support `{{! ... }}` — leading `!` after `{{` marks raw (sanitized) HTML insertion
            if (code.length > 0 && code[0] === '!') {
                isRaw = true;
                code = code.slice(1).trim();
            } else if (doubleRaw) {
                // If configured, treat plain `{{ ... }}` as raw (sanitized)
                isRaw = true;
            }
        }
        ...
        try {
            ...
            else { parts.push({ type: isRaw ? 'raw' : 'expr', value: code }); }
        } catch (err) {
    ...
    const compiled = { parts, options: { doubleBraceRaw: doubleRaw } };
        ...
        if (part.type === 'text') out += part.value;
            ...
            try {
                ...
                if (value !== undefined && value !== null) {
                    // Sanitization removed intentionally: insert raw HTML as-is
                    out += String(value);
                }
            } catch (err) { /* skip errors */ }
        } else { /* unsafe */ }
